...
class PatternFinder(BaseAgent):
    """Агент для поиска закономерностей в данных пользователя."""
    ...
    async def execute(self, input_data: AgentInput) -> AgentOutput:
        ...
        messages = [
            {"role": "system", "content": self.config.system_prompt},
            {
                "role": "user",
                "content": f"""
Проанализируй данные пользователя для поиска закономерностей:
...
1. Тренды прогресса в тренировках
2. Тренды питания
3. Закономерности в субъективных оценках
4. Корреляции между активностью и результатами
